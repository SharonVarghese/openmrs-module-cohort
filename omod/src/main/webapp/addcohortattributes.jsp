...
<select name="cohortAttributeTypeId" onchange="document.getElementById('form1').method='GET'; document.getElementById('form1').submit()" >
<option value=""></option>
<c:forEach var="attype" items="${attypes}">
<option value="${attype.cohortAttributeTypeId}"<c:if test="${attype ==cohortatt.cohortAttributeType}">selected</c:if> >${attype}</option>
</c:forEach>
