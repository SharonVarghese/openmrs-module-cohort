...
<option value="${names}"<c:if test="${names ==cohortatt.cohortMember}">selected</c:if>>${names}</option>
</c:forEach>
</select>
...
<option value="${format}"<c:if test="${format ==cohortatt.cohortMemberAttributeType}">selected</c:if>>${format}</option>
</c:forEach>
</select>
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
</spring:bind>
