...
<select name="cohortMemberAttributeTypeId" onchange="document.getElementById('form2').method='GET'; document.getElementById('form2').submit()">
<option value=""></option>
<c:forEach var="attype" items="${attypes}">
<option value="${attype.cohortMemberAttributeTypeId}"<c:if test="${attype ==cohortatt.cohortMemberAttributeType}">selected</c:if>>${attype}</option>
</c:forEach>
