...
<option value="${names}"<c:if test="${names ==cohortencounters.cohort}">selected</c:if>>${names}</option>
</c:forEach>
</select>	
...
<option value="${enctype}"<c:if test="${enctype ==cohortencounters.encounterType}">selected</c:if>>${enctype}</option>
</c:forEach>
</select>
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
</spring:bind>	
...
<option value="${location}"<c:if test="${location ==cohortencounters.location}">selected</c:if>>${location}</option>
</c:forEach>
</select>
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
</spring:bind>
...
<input type="text" name="encounterDateTime" size="10" onFocus="showCalendar(this,60)"
						   id="encounterDateTime" value="${status.value}" /><i style="font-weight: normal; font-size: 0.8em;">(<openmrs:message code="general.format"/>: <openmrs:datePattern /></i><br/><br/>
<c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
</spring:bind>
